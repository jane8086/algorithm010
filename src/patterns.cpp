...
*/
int create_patterns_novel(int screen_Width, int screen_Hight, int period_sum,
  ...
  for (int k = 0; k < step_number; k++) {
    ...
    for (int i = 0; i < grayim.rows; ++i)
      for (int j = 0; j < grayim.cols; ++j) {
        ...
        grayim.at<uchar>(i, j) =
            offset +
            int(amplitude * (cos((float)period_number * interval_angle +
                                 (k - 1) * 2 * CV_PI / 3.0)));
      }
...
input: screen_Width: resolution U
       screen_Hight: resolution V
       period_sum: sum of the periods
       image_type: 1 for creating images, which are used to determine column
number, other numbers for creating images, which are used to determine row
number. output: vector of mats
*/
int create_patterns_phaseshift_general(int screen_Width, int screen_Hight,
                                       int period_sum, int step_number,
                                       vector<Mat> &patterns_phaseshift) {
  ...
  for (int k = 1; k < step_number + 1; k++) {
    ...
    for (int i = 0; i < grayim.rows; ++i)
      for (int j = 0; j < grayim.cols; ++j) {
        grayim.at<uchar>(i, j) =
            offset +
            int(amplitude * (cos(period_sum * 2 * pi * j / (float)screen_Width -
                                 2 * pi * k / (float)step_number)));
      }
  ...
  for (int k = 1; k < step_number + 1; k++) {
    ...
    for (int i = 0; i < grayim.rows; ++i)
      for (int j = 0; j < grayim.cols; ++j) {
        grayim.at<uchar>(i, j) =
            offset +
            int(amplitude * (cos(period_sum * 2 * pi * i / float(screen_Hight) -
                                 2 * pi * k / (float)step_number)));
      }
...
number. output: vector of mats
*/
int create_patterns_phaseshift(int screen_Width, int screen_Hight,
                               int period_sum,
                               vector<Mat> &patterns_phaseshift) {
...
input: screen_Width: resolution U
       ...
       image_type: 1 for creating images, which are used to determine column
number, other numbers for creating images, which are used to determine row
number. output: one image, whose three channels have different phase shift.
Order: RGB (-2pi/3 0 2p/3)
*/
int create_patterns_colorphaseshift(int screen_Width, int screen_Hight,
                                    int period_sum,
                                    vector<Mat> &patterns_colorphaseshift) {
  int step_number; //, offset, amplitude ;
  step_number = 3;
  // offset = 122;
  // amplitude = 122;
...
// monitor width and height and period n -->vertical mode
int create_patterns_graycodevertical(int screen_Width, int screen_Hight,
                                     int perdiod_sum,
                                     vector<Mat> &graycodevertical) {
  ...
  Mat gray_pattern(screen_Hight, screen_Width,
                   CV_8UC1); // create a blank pattern
  ...
  for (int p = 1; p <= perdiod_sum; p++) // gray_code_pattern with n period
  {
    ...
    for (int x = 1; x <= arrsiz; x++) //
    {
      ...
      tem = s[perdiod_sum - 1];
      if ((tem - 48) == 0) {
        gray_pattern(Range(0, screen_Hight),
                     Range((x - 1) * unit_w, x * unit_w)) =
            0; // fill black in pattern
      }
    }
...
int create_patterns_graycodehorizontal(
    int screen_Width, int screen_Hight, int period_sum,
    vector<Mat> &patterns_graycode_horizontal) {
  // adjust period
  ...
  for (int p = 1; p <= period_sum; p++) // gray_code_pattern with n period
  {
    ...
    for (int x = 1; x <= arrsiz; x++) {
      ...
      tem = s[period_sum - 1];
      if ((tem - 48) == 0) {
        gray_pattern(Range((x - 1) * unit_h, x * unit_h),
                     Range(0, screen_Width)) = 0; // fill black in pattern
      }
    }
...
int create_patterns_offset(int screen_Width, int screen_Hight,
                           vector<Mat> &output) {
...
int create_patterns_all(int screen_Width, int screen_Hight, int period_sum,
                        vector<Mat> &patterns, int amount_shifts,
                        int color_pattern, int novel_method) {
  ...
  if (color_pattern) {
    ...
    create_patterns_colorphaseshift(screen_Width, screen_Hight, period_sum,
                                    patterns);
  } else {

    if (create_patterns_phaseshift_general(screen_Width, screen_Hight,
                                           period_sum, amount_shifts, patterns))
      return -1;
  }
  ...
  if (novel_method) {
    ...
    if (create_patterns_graycodehorizontal(screen_Width, screen_Hight,
                                           period_sum, patterns))
      return -1;
    if (create_patterns_graycodevertical(screen_Width, screen_Hight, period_sum,
                                         patterns))
      return -1;
  }
